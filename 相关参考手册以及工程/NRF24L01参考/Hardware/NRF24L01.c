#include "stm32f10x.h" // Device header
#include "NRF24L01_define.h"
#include "Delay.h"

// 发送地址和接收地址(5字节)
uint8_t T_ADDR[5] = {0x59, 0x51, 0x52, 0x21, 0x19};
uint8_t R_ADDR[5] = {0x59, 0x51, 0x52, 0x21, 0x19};

/**
 * @brief  写MOSI引脚电平(SPI数据输出线)
 * @param  Value: 0-低电平, 1-高电平
 * @retval 无
 */
void W_MOSI(uint8_t Value)
{
	GPIO_WriteBit(MOSI_Port, MOSI_Pin, (BitAction)Value);
}

/**
 * @brief  写SCK引脚电平(SPI时钟线)
 * @param  Value: 0-低电平, 1-高电平
 * @retval 无
 */
void W_SCK(uint8_t Value)
{
	GPIO_WriteBit(SCK_Port, SCK_Pin, (BitAction)Value);
}

/**
 * @brief  写CSN引脚电平(SPI片选信号,低电平有效)
 * @param  Value: 0-选中NRF24L01, 1-取消选中
 * @retval 无
 */
void W_CSN(uint8_t Value)
{
	GPIO_WriteBit(CSN_Port, CSN_Pin, (BitAction)Value);
}

/**
 * @brief  写CE引脚电平(NRF24L01使能信号)
 * @param  Value: 0-待机模式, 1-发送/接收模式
 * @retval 无
 */
void W_CE(uint8_t Value)
{
	GPIO_WriteBit(CE_Port, CE_Pin, (BitAction)Value);
}

/**
 * @brief  读IRQ引脚电平(中断信号,低电平有效)
 * @param  无
 * @retval 0-有中断, 1-无中断
 */
uint8_t R_IRQ(void)
{
	return GPIO_ReadInputDataBit(IRQ_Port, IRQ_Pin);
}

/**
 * @brief  读MISO引脚电平(SPI数据输入线)
 * @param  无
 * @retval 0-低电平, 1-高电平
 */
uint8_t R_MISO(void)
{
	return GPIO_ReadInputDataBit(MISO_Port, MISO_Pin);
}

/**
 * @brief  NRF24L01引脚初始化
 * @param  无
 * @retval 无
 * @note   配置SPI引脚(MOSI、SCK、CSN、CE)为推挽输出
 *         配置中断引脚(IRQ)和数据输入引脚(MISO)为上拉输入
 */
void NRF24L01_Pin_Init(void)
{

	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOB, ENABLE);
	GPIO_InitTypeDef GPIO_InitStructure;

	// 配置成推挽输出
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Pin = MOSI_Pin;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(MOSI_Port, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Pin = SCK_Pin;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SCK_Port, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Pin = CSN_Pin;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(CSN_Port, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Pin = CE_Pin;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(CE_Port, &GPIO_InitStructure);

	// 配置成上拉输入
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO_InitStructure.GPIO_Pin = IRQ_Pin;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(IRQ_Port, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO_InitStructure.GPIO_Pin = MISO_Pin;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(MISO_Port, &GPIO_InitStructure);
}

/**
 * @brief  SPI交换一个字节(软件模拟SPI通信)
 * @param  Byte: 要发送的字节数据
 * @retval 接收到的字节数据
 * @note   采用模式0(CPOL=0, CPHA=0): 空闲时SCK为低电平,第一个时钟沿采样
 *         高位先行(MSB First),每次循环发送和接收1位数据
 */
uint8_t SPI_SwapByte(uint8_t Byte)
{
	uint8_t i, ByteReceive = 0x00;
	for (i = 0; i < 8; i++)
	{
		W_MOSI(Byte & (0x80 >> i));
		W_SCK(1);
		if (R_MISO() == 1)
		{
			ByteReceive = ByteReceive | (0x80 >> i);
		}
		W_SCK(0);
	}
	return ByteReceive;
}

/**
 * @brief  写NRF24L01寄存器(单字节)
 * @param  Reg: 寄存器操作指令(操作码+寄存器地址),例如: W_REGISTER + STATUS
 * @param  Value: 要写入寄存器的数据值
 * @retval 无
 * @note   通过SPI发送命令字节和数据字节来完成寄存器写操作
 */
void W_Reg(uint8_t Reg, uint8_t Value)
{
	W_CSN(0);			 // 表示选中NRF24L01
	SPI_SwapByte(Reg);	 // 交换的第一个字节就是寄存器
	SPI_SwapByte(Value); // 交换的第二个字节就是交换的数据
	W_CSN(1);			 // 停止选中NRF24L01
}

/**
 * @brief  读NRF24L01寄存器(单字节)
 * @param  Reg: 寄存器操作指令(操作码+寄存器地址),例如: R_REGISTER + STATUS
 * @retval 读取到的寄存器数据值
 * @note   发送读命令后,需要再发送一个NOP指令来接收数据
 */
uint8_t R_Reg(uint8_t Reg)
{
	uint8_t Value;
	W_CSN(0);				   // 表示选中NRF24L01
	SPI_SwapByte(Reg);		   // 交换的第一个字节就是寄存器
	Value = SPI_SwapByte(NOP); // 交换的第二个字节就是交换的数据
	W_CSN(1);				   // 停止选中NRF24L01
	return Value;
}

/**
 * @brief  写NRF24L01多个寄存器
 * @param  Reg: 寄存器操作指令(操作码+寄存器地址)
 * @param  Buf: 指向要写入数据的缓冲区指针
 * @param  Len: 要写入的数据长度(字节数)
 * @retval 无
 * @note   用于写入地址寄存器(5字节)或有效载荷数据(最多32字节)
 */
void W_Buf(uint8_t Reg, uint8_t *Buf, uint8_t Len)
{
	uint8_t i;
	W_CSN(0); // 选中NRF24L01
	SPI_SwapByte(Reg);
	for (i = 0; i < Len; i++)
	{
		SPI_SwapByte(Buf[i]);
	}
	W_CSN(1); // 停止选中NRF24L01
}

/**
 * @brief  读NRF24L01多个寄存器
 * @param  Reg: 寄存器操作指令(操作码+寄存器地址)
 * @param  Buf: 指向接收数据的缓冲区指针
 * @param  Len: 要读取的数据长度(字节数)
 * @retval 无
 * @note   用于读取地址寄存器(5字节)或接收的有效载荷数据(最多32字节)
 */
void R_Buf(uint8_t Reg, uint8_t *Buf, uint8_t Len)
{
	uint8_t i;
	W_CSN(0); // 选中NRF24L01
	SPI_SwapByte(Reg);
	for (i = 0; i < Len; i++)
	{
		Buf[i] = SPI_SwapByte(NOP);
	}
	W_CSN(1); // 停止选中NRF24L01
}

/// @brief NRF24L01模块初始化
/// @param  无
void NRF24L01_Init(void)
{
	NRF24L01_Pin_Init();

	W_CE(0);

	W_Buf(W_REGISTER + TX_ADDR, T_ADDR, 5);	   // 配置发送地址
	W_Buf(W_REGISTER + RX_ADDR_P0, R_ADDR, 5); // 配置接收通道0
	W_Reg(W_REGISTER + CONFIG, 0x0F);		   // 配置成接收模式
	W_Reg(W_REGISTER + EN_AA, 0x01);		   // 通道0开启自动应答
	W_Reg(W_REGISTER + RF_CH, 0x00);		   // 配置通信频率2.4G
	W_Reg(W_REGISTER + RX_PW_P0, 32);		   // 配置接收通道0接收的数据宽度32字节
	W_Reg(W_REGISTER + EN_RXADDR, 0x01);	   // 接收通道0使能
	W_Reg(W_REGISTER + SETUP_RETR, 0x1A);	   // 配置580us重发时间间隔,重发10次
	W_Reg(FLUSH_RX, NOP);

	W_CE(1);
}

/**
 * @brief  NRF24L01接收数据
 * @param  Buf: 指向接收数据缓冲区的指针(至少32字节)
 * @retval 无
 * @note   检查STATUS寄存器的RX_OK标志位,如果有数据则读取
 *         读取后清空RX FIFO并清除中断标志
 */
void Receive(uint8_t *Buf)
{
	uint8_t Status;
	Status = R_Reg(R_REGISTER + STATUS);
	// 对寄存器进行检查是否接收完了数据
	if (Status & RX_OK)
	{
		R_Buf(R_RX_PAYLOAD, Buf, 32);
		W_Reg(FLUSH_RX, NOP);
		W_Reg(W_REGISTER + STATUS, Status);
		Delay_us(150);
	}
}

/**
 * @brief  NRF24L01发送数据
 * @param  Buf: 指向要发送数据的缓冲区指针(32字节)
 * @retval MAX_TX: 达到最大重发次数,发送失败
 *         TX_OK: 发送成功并收到应答
 * @note   切换到发送模式,等待中断信号,检查发送状态
 *         发送完成后自动切换回接收模式(在主程序中处理)
 */
uint8_t Send(uint8_t *Buf)
{
	uint8_t Status;
	W_Buf(W_TX_PAYLOAD, Buf, 32); // 在发送数据缓存器发送要发送的数据

	W_CE(0);
	W_Reg(W_REGISTER + CONFIG, 0x0E); // 切换到发送模式(PWR_UP=1, PRIM_RX=0)
	W_CE(1);

	while (R_IRQ() == 1)
		; // 等待中断
	Status = R_Reg(R_REGISTER + STATUS);

	if (Status & MAX_TX) // 如果发送达到最大次数
	{
		W_Reg(FLUSH_TX, NOP);				// 清除发送数据缓存器
		W_Reg(W_REGISTER + STATUS, Status); // 中断位写1清除中断
		return MAX_TX;
	}
	if (Status & TX_OK) // 如果发送成功,接收到应答信号
	{
		W_Reg(W_REGISTER + STATUS, Status); // 清除中断
		return TX_OK;
	}
}
